const navLinks = [
  {
    text: "Home",
    url: "#",
    current: true,
  },
  {
    text: "About",
    url: "#",
    current: false,
  },
  {
    text: "Services",
    url: "#",
    current: false,
  },
  {
    text: "Blog",
    url: "#",
    current: false,
  },
  {
    text: "Contact",
    url: "#",
    current: false,
  },
  {
    text: "Portfolio",
    url: "#",
    current: false,
  }
];

const services = [
  {
    src: './images/Group-247.png',
    title: "Data Analysis",
    text: 'When, while the lovely valley teems with vapour around meand the meridian sun strikes the upper',
  },
  {
    src: "/images/Group-567.png",
    title: "SEO Optimization",
    text: 'When, while the lovely valley teems with vapour around meand the meridian sun strikes the upper',
  },
  {
    src: "./images/Group-538.png",
    title: "Security Data",
    text: 'When, while the lovely valley teems with vapour around meand the meridian sun strikes the upper',
  },
  {
    src: "./images/Group-566.png",
    title: "Branding Strategy",
    text: 'When, while the lovely valley teems with vapour around meand the meridian sun strikes the upper',
  }
];

const works = [
  {
    src: "./images/8wa60okr-1-790x576.jpg",
    title: 'Basket of Flower on table',
    role:'Branding Strategy'
  },
  {
    src: "./images/DRY-1-790x576.jpg",
    title: 'Purinky Products',
    role:'Digital Experience'
  },
  {
    src: "./images/a247b00b-3621-470f-b4b8-b3ac46f25907-1-790x576.jpg",
    title: 'Satisfy Poster',
    role:'Branding Strategy'
  },
  {
    src: "./images/84316050-0af0-49db-a53a-241d47ddad0e-2-790x576.jpg",
    title: 'Mock-up Template',
    role:'Branding Strategy'
  },
];

const people = [
  {
    src: "./images/szabo-viktor-1266895-unsplash-1024x1024.png",
    name: 'Michkel Anegl',
    role: 'Marketing Management'
  },
  {
    src: "./images/philipe-cavalcante-539505-unsplash-1024x1024.png",
    name: 'Michkel Anegl',
    role: 'Marketing Management'
  },
  {
    src: "./images/fabio-spinelli-695744-unsplash-1024x1024.png",
    name: 'Michkel Anegl',
    role: 'Marketing Management'
  },
];

const footerLinks = [
  {
    title: 'Quick Links',
    links: [
      {
        text: "Services",
        url: "#",
      },
      {
        text: "Contact",
        url: "#",
      },
      {
        text: "Blog",
        url: "#",
      }
    ]
  },
  {
    title: 'Resoirces',
    links: [
      {
        text: "Art Design",
        url: "#",
      },
      {
        text: "Computer",
        url: "#",
      },
      {
        text: "IT News",
        url: "#",
      },
      {
        text: "Networking",
        url: "#",
      },
      {
        text: "Web Security",
        url: "#",
      }
    ]
  },
  {
    title: 'Contact US',
    links: [
      {
        text: "Address:",
        info: "Patricia C. Amedee 4401 Waldeck Street Grapevine Nashville, Tx 76051",
        url: "#",
      },
      {
        text: "Phone:",
        info: "+99 (0) 101 0000 888",
        url: "#",
      },
      {
        text: "Email:",
        info: "Info@yourdomain.com",
        url: "#",
      },
    ]
  }
];

export {navLinks, services, works, people, footerLinks};